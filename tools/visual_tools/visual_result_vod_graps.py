...
save_path = './visual_result/vod_graps'
infos = np.load(data_root / 'kitti_infos_val.pkl', allow_pickle=True)
det_result = np.load('./output/tools/cfgs/experiments/1gauss_4uniform/default/eval/epoch_19/val/default/result.pkl', allow_pickle=True)
...
def plot_gt_bev(gt_boxes, color=np.array([255, 0, 0]) / 256, facecolor=None):
    ...
    for gt_box in gt_boxes:
        ...
        # if gt_box[3] * gt_box[4] > 2:
        #     box_corner = box2corner_3d(gt_box)
        #     corners = np.array(box_corner)[:, :2]
        #     middle = corners[:2].mean(0)
        #     center = gt_box[:2]
        #     middle[0] += w / 2
        #     center[0] += w / 2
        #     middle[1] += h / 2
        #     center[1] += h / 2
        #     line = np.stack([middle, center])
        #     plt.plot(line[:, 0], line[:, 1], color=color,  linestyle='solid')
