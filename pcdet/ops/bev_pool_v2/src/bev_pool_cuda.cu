...
__global__ void bev_pool_v2_kernel(int c, int n_intervals,
  ...
  int idx = blockIdx.x * blockDim.x + threadIdx.x; // 当前所在thread的index = block index * 每个block包含的thread数量 + thread index
  int index = idx / c; // 当前在处理哪个voxel
  int cur_c = idx % c; // 当前在处理voxel的哪个通道
  if (index >= n_intervals) return;
  int interval_start = interval_starts[index]; //voxel点集的开始位置
  int interval_length = interval_lengths[index]; //点集的点的数量
  float psum = 0;
  const float* cur_depth; //当前所取的深度特征指针
  const float* cur_feat; // 当前所取的特征指针
...
__global__ void bev_pool_grad_kernel(int c, int n_intervals,
  ...
  float grad_sum;
  for(int i = 0; i < interval_length; i++){
    cur_rank = ranks_bev + interval_start + i; // bev特征图的值（指针）
