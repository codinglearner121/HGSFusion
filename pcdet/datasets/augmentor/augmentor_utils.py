...
def random_image_flip_horizontal(image, gt_boxes, calib, prob=0.5, gt_boxes2d=None):
    """
    ...
    enable = np.random.choice([False, True], replace=False, p=[1.0 - prob, prob])
    ...
    if enable:
        ...
        aug_gt_boxes2d = gt_boxes2d.copy()
        aug_gt_boxes2d[:, [0, 2]] = width - gt_boxes2d[:, [2, 0]]
        # from matplotlib import pyplot as plt
        # fig = plt.figure()
        # ax = fig.add_subplot(211)
        # plt.imshow(image)
        # for gt_box in gt_boxes2d:
        #     ax.add_patch(plt.Rectangle((gt_box[0], gt_box[1]), gt_box[2] - gt_box[0], gt_box[3] - gt_box[1], edgecolor='r', facecolor='none'))
        # ax = fig.add_subplot(212)
        # plt.imshow(aug_image)
        # for gt_box in aug_gt_boxes2d:
        #     ax.add_patch(plt.Rectangle((gt_box[0], gt_box[1]), gt_box[2] - gt_box[0], gt_box[3] - gt_box[1], edgecolor='r', facecolor='none'))
        # plt.savefig("/media/yellowstone/databig/code/OpenPCDet_VoD_new/test.jpg")
        # pass
    ...
    return aug_image, aug_gt_boxes, aug_gt_boxes2d, enable
