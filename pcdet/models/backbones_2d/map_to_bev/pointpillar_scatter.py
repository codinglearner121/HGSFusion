...
class PointPillarScatter(nn.Module):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        # coords          : [N, B] [batch_size_index, 坐标*3]
        # N:整个batch size的总点数
        # B:batch size
        ...
        for batch_idx in range(batch_size):
            spatial_feature = torch.zeros(
                ...
                self.nz * self.nx * self.ny, # 432*496=214272
                dtype=pillar_features.dtype,
                device=pillar_features.device)

            batch_mask = coords[:, 0] == batch_idx
            this_coords = coords[batch_mask, :] # [n, 4] n:单个样本的点数
        ...
        batch_spatial_features = torch.stack(batch_spatial_features, 0) # [2,64,102400]
        batch_spatial_features = batch_spatial_features.view(batch_size, self.num_bev_features * self.nz, self.ny, self.nx) # [2,64,320,320]
        batch_dict['spatial_features'] = batch_spatial_features # [16, 64, 320, 320] vod
        return batch_dict
