...
class PillarVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        f_cluster = voxel_features[:, :, :3] - points_mean #[N, 32, 3] 每个点与平均点的差距

        f_center = torch.zeros_like(voxel_features[:, :, :3]) #[N, 32, 3] 每个点与中心点的差距
        ...
        voxel_count = features.shape[1] #每个voxel的最大点数
        mask = self.get_paddings_indicator(voxel_num_points, voxel_count, axis=0) #[N, 32] 差几个（是否要）zero - padding
        mask = torch.unsqueeze(mask, -1).type_as(voxel_features)
        features *= mask # [20538, 32, 10]
        for pfn in self.pfn_layers:
            features = pfn(features)
        features = features.squeeze() # [20538, 1, 64]
