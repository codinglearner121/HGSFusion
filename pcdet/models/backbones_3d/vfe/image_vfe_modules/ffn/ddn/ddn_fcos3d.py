...
class DDN_FCOS3D(nn.Module):

    def __init__(self, feat_extract_layer, num_classes, pretrained_path=None, aux_loss=None, backbone_name=None, 
                 pretrained_path_fcos3d=None):
        """
        ...
        self.pretrained_path_fcos3d = pretrained_path_fcos3d
    ...
    def get_model(self, constructor):
        ...
        if self.pretrained_path is not None:
            ...
            pretrained_dict_deeplabv3 = torch.load(self.pretrained_path)
            pretrained_dict_fcos3d = torch.load(self.pretrained_path_fcos3d)['state_dict']
            pretrained_dict_deeplabv3 = self.filter_pretrained_dict(model_dict=model_dict,
                                                          pretrained_dict=pretrained_dict_deeplabv3)
            for x in list(pretrained_dict_fcos3d.keys()):
                if x.find('backbone') == -1:
                    pretrained_dict_fcos3d.pop(x)

            # Update current model state dict
            model_dict.update(pretrained_dict_deeplabv3)
            model_dict.update(pretrained_dict_fcos3d)
